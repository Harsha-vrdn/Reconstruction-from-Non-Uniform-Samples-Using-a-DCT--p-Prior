...
def run_experiments(image_paths, sampling_ratios=[0.1,0.2,0.3,0.5],
    ...
    for img_path in image_paths:
        ...
        for r in sampling_ratios:
            ...
            # sampled measurements (noisy)
            sampled = mask * x_true
            # add Gaussian noise to observed pixels to achieve target SNR
            # compute noise scale
            ...
            # place Gaussian noise at mask==1 so that overall noise norm matches eta_norm
            ...
            for p in p_list:
                for lam in lambda_grid:
                    x_hat, diag = mm_cg_reconstruct(m, mask, lam, p=p, eps=eps,
                                                    tol_cg=1e-6, tol_mm=1e-4, max_mm_iters=200, verbose=False)
                    ...
                    if cur_psnr > best['psnr']:
                        best = {'p': p, 'lam': lam, 'psnr': cur_psnr, 'rel': cur_rel, 'x_hat': x_hat, 'diag': diag}
            ...
            plt.imsave(os.path.join(out_dir, f"{out_prefix}_recon_p{best['p']}_lam{best['lam']}.png"),
            ...
            print(f"{base_name} r={r}: best p={best['p']}, lam={best['lam']}, PSNR={best['psnr']:.2f} dB, rel={best['rel']:.4f}")
